...
| `HTTP_SHUTDOWN_TIMEOUT` | `10s` | grace period for in-flight requests on SIGTERM/SIGINT; remaining connections are closed when it elapses and the process exits 1. Must be at least `HTTP_REQUEST_TIMEOUT`, or every request in flight at a deployment would be cut off |
...
| `PROCESSING_JOB_LEASE` | `15m` | how long a dispatch may hold a job in `PROCESSING`; a job still `PROCESSING` after its lease expired was interrupted by a crash and is failed by the sweep with `PROCESSING_INTERRUPTED`. Must be at least twice `HTTP_REQUEST_TIMEOUT` plus `PROCESSING_FAILURE_RECORD_TIMEOUT` |
| `PROCESSING_LEASE_SWEEP_INTERVAL` | `1m` | how often expired job leases are swept (1s to 1h) |
...
- **Leases.** A dispatch holds the job under a lease (`PROCESSING_JOB_LEASE`) far longer than it can legitimately last. A job still `PROCESSING` after its lease expired belongs to a gateway that died mid-dispatch, which no graceful shutdown can drain; every replica sweeps expired leases (`PROCESSING_LEASE_SWEEP_INTERVAL`) and fails such a job with `PROCESSING_INTERRUPTED`, keeping its attempt count and timestamps, so an interrupted job is never left in flight (docs/RESILIENCE.md). A job the processor finished but whose results could not be written is failed in the request itself with `PROCESSING_RESULTS_NOT_STORED`.
