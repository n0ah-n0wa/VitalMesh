...
| Toolchain drift | the first run of the pipeline failed in the dependency scan: govulncheck v1.8.0 requires Go 1.26 and CI runs with `GOTOOLCHAIN=local` (set by `setup-go`), while the local run had passed on a silently downloaded toolchain | govulncheck pinned to v1.7.0, the last release built for Go 1.25; the Makefile now exports `GOTOOLCHAIN=local` so local runs fail the same way CI does. The failure was reproduced locally with the old pin, and the fix verified from a clean checkout |
