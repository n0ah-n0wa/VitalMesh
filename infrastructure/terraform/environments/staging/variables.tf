...
variable "ingress_domain_name" {
  description = "Hostname the Ingress serves; an ACM certificate is issued for it. Must equal the host in infrastructure/kubernetes/overlays/staging/ingress.yaml (currently api.staging.vitalmesh.example, a placeholder until a domain exists)."
  type        = string
  default     = null
}
