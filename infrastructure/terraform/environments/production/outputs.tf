...
output "kubernetes_namespace" {
  description = "Namespace the application deploys into; matches infrastructure/kubernetes/overlays/production."
  value       = module.platform.kubernetes_namespace
}

output "deploy_role_arn" {
  description = "Role the CD job assumes through GitHub OIDC, from the production GitHub environment only."
...
output "nat_public_ips" {
  description = "Addresses production's outbound traffic comes from."
