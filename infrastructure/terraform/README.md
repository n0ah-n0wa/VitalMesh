...
2. Configure the repository as [docs/GITHUB_CONFIGURATION.md](../../docs/GITHUB_CONFIGURATION.md) says: the variables that name the account and the state bucket, the `staging` and `production` environments (production with required reviewers), branch protection on `main`. The deploy roles trust exactly those environment names, so a job outside them cannot assume the roles.
...
| the end-to-end test account's password (staging only) | an ephemeral `random_password`, written with `secret_string_wo` | `vitalmesh/staging/e2e`; the deploy job creates the account in the cluster with it, the E2E job signs in with it |
...
| `deploy_parameter_name` | the SSM parameter (`/vitalmesh/<env>/deploy`) the deploy job reads for endpoints, secret ARNs, the ingress host and (staging) the E2E account; Terraform keeps it current, so a restore that renames the database needs no GitHub change |
| `e2e_secret_arn` | staging only: the secret holding the end-to-end test account's password (`vitalmesh/staging/e2e`); production refuses to create one |
...
`apply` works the same way, run by a person. There is no CI apply role (see below).

**In CI.** Once the repository variables are set, `.github/workflows/terraform-plan.yml` runs a real plan for both environments on every pull request and push that touches the Terraform, through the read-only plan role and GitHub OIDC (no keys), and puts the change list in the run's summary. The plan role can read configuration and state and nothing else; a plan cannot change anything.
...
- **No tflint.** Its AWS ruleset would catch an instance type that does not exist without an account; the CI plan (once the account is configured) catches the same thing later and louder.
