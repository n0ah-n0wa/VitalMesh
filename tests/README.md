...
- `services/api-gateway/tests/stack`, run by `make stack-test`, includes the automated failure-injection cases (PostgreSQL, Redis and processor outages, processor timeout, transient network faults, bounded retries, container restarts, a gateway killed mid-job; see [docs/FAILURE_MODES.md](../docs/FAILURE_MODES.md) and [docs/RESILIENCE.md](../docs/RESILIENCE.md)): the real containerized stack (both images, PostgreSQL, Redis) started clean by `scripts/stack-test.sh`, driven purely over HTTP, with the failure cases injected by stopping, pausing and restarting containers. See "The stack suite" in [docs/DEVELOPMENT.md](../docs/DEVELOPMENT.md).
